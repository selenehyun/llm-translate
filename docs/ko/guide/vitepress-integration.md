...
llm-translate는 번역된 문서 구조를 기반으로 VitePress i18n 설정을 자동으로 생성하는 헬퍼 함수를 제공합니다.

## 개요

`llm-translate dir` 으로 문서를 번역한 후, 내장된 VitePress 헬퍼를 사용하여 각 로케일에 대한 네비게이션 및 사이드바 설정을 자동으로 생성할 수 있습니다.
...
::: tip 절대 경로를 사용하는 이유는?
VitePress 설정은 프로젝트 루트에서 실행되므로,`'./docs'` 와 같은 상대 경로는 올바르게 해석되지 않을 수 있습니다.`import.meta.url` 를 사용하면 설정 파일 위치를 기준으로 경로가 계산됩니다.
:::

이 함수는 다음을 수행합니다:
1. `./docs` 디렉토리 구조를 스캔합니다
2. 사이드바 디렉토리(guide, api, cli 등)를 자동으로 감지합니다
3. 각 로케일에 대한 nav 및 사이드바를 생성합니다
4. UI 요소에 기본 번역을 적용합니다
...
nav는 수동으로 설정하고 사이드바만 자동으로 생성하려는 경우:
...
로케일 디렉토리를 스캔하여 사용 가능한 로케일을 자동으로 감지합니다:
...
사이드바에 표시되어야 하는 디렉토리를 자동으로 감지합니다:
...
- **절대 경로 사용**: 기본 사용법에 표시된 대로 항상 `import.meta.url` 를 사용하여 docs 디렉토리 경로를 해석합니다. VitePress가 프로젝트 루트에서 실행되므로 상대 경로는 올바르게 작동하지 않을 수 있습니다.
- 로케일 디렉토리는 2자리 코드를 사용해야 합니다(예:`ko `,` ja `,` zh`)
- 헬퍼는 번역된 문서가 소스 구조를 반영한다고 가정합니다
- 사용자 정의 nav 항목(외부 링크, 드롭다운)은 수동으로 설정해야 합니다
