...
`llm-translate dir` でドキュメントを翻訳した後、組み込みのVitePress ヘルパーを使用して、各ロケールのナビゲーションとサイドバー設定を自動生成できます。
...
::: tip 絶対パスを使用する理由
VitePress設定はプロジェクトルートから実行されるため、 `'./docs'` のような相対パスは正しく解決されない場合があります。 `import.meta.url` を使用することで、設定ファイルの場所を基準としてパスが計算されることが保証されます。
:::

これにより、以下の処理が実行されます：
1. `./docs` ディレクトリ構造をスキャンします
2. サイドバーディレクトリ（guide、api、cliなど）を自動検出します
3. 各ロケールのnavとサイドバーを生成します
4. UI要素のデフォルト翻訳を適用します
...
ロケールディレクトリをスキャンして利用可能なロケールを自動検出します：
...
ヘルパーはこの順序でページタイトルを抽出します：
1. Frontmatterの `title` フィールド
2. ファイル内の最初の `#` 見出し
3. ファイル名をタイトルケースに変換したもの
...
- **絶対パスを使用してください**：基本的な使用方法に示されているように、常に `import.meta.url` を使用してdocsディレクトリパスを解決してください。VitePresはプロジェクトルートから実行されるため、相対パスは正しく機能しない場合があります。
- ロケールディレクトリは2文字コード（例：`ko ` 、`ja ` 、`zh`）を使用する必要があります
- ヘルパーは翻訳されたドキュメントがソース構造をミラーリングしていることを前提としています
- カスタムnavアイテム（外部リンク、ドロップダウン）は手動で設定する必要があります
